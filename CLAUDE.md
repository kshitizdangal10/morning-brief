...
The 34 highest-stakes entries (all exams, major project/paper deadlines, mandatory discussion posts, attendance-required events — deliberately excluding routine weekly SmartBook/reading checkpoints to avoid calendar clutter) are also synced as all-day Google Calendar events with a 1-day-before popup reminder, on the student's `kshitizdangal10@gmail.com` calendar. This was a one-time manual sync via calendar MCP tools, not a script — if the schedule data changes (a syllabus gets revised) or a new course is added, the calendar copy will drift and needs to be manually reconciled; there's no automated sync keeping them in lockstep.
